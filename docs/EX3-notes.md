...
- **Mobile-responsive** — below `md` the sidebar becomes an off-canvas drawer (backdrop + Esc + close-on-nav), with a bottom tab bar and `env(safe-area-inset-*)` padding.
