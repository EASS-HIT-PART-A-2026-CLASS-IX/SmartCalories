...
def populate_starter(session: Session, user_uid: str) -> dict[str, int]:
    """Lightweight playground data for ANY user — idempotent.

    Used after a guest (Firebase anonymous) sign-in so a brand-new browser doesn't see an
    empty product. Skips entirely when the user already has FoodEntry rows.
    """
    counts = {"food_entry": 0, "goals": 0, "preferences": 0}

    existing = session.exec(
        select(FoodEntry).where(FoodEntry.user_uid == user_uid).limit(1)
    ).first()
    if existing is not None:
        return counts

    if session.get(UserGoals, user_uid) is None:
        session.add(
            UserGoals(
                user_uid=user_uid,
                daily_kcal=2000,
    ...
    if session.get(Preferences, user_uid) is None:
        session.add(
            Preferences(
                user_uid=user_uid, theme="system", language="en", units="metric", notifications={}
            )
        )
        counts["preferences"] += 1
    ...
    for offset in range(5):
        ...
        sn_name, sn_kcal, sn_p, sn_c, sn_f = rng.choice(_SNACKS)
        food_rows.append(
            FoodEntry(
                user_uid=user_uid,
                name=sn_name,
                ...
                fat_g=sn_f,
                meal=Meal.snack,
                eaten_at=day.replace(hour=16, minute=rng.randint(0, 59)),
                source=Source.manual,
            )
        )
    session.bulk_save_objects(food_rows)
